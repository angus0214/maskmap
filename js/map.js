...
function setBasicMap() {
    ...
    for (let i = 0; data.length > i; i++) {
        ...
        markers.addLayer(L.marker([data[i].geometry.coordinates[1], data[i].geometry.coordinates[0]], { icon: selecltIconColor(maskAdult, maskChild) }).bindPopup(
            `
                <div class="popupBox">
                <h2>${title}</h2>
                <p>${address}</p>
                <p>連絡電話｜${tel}</p>
                <div class="maskBox">
                    <span class="btn ${maskCount(maskAdult, 'adult')}">成人口罩<br> ${maskAdult} 個</span>
                    <span class="btn ${maskCount(maskChild, 'kid')}">兒童口罩 ${maskChild} 個</span>
                </div>
...
function getData() {
    let data;
    $.getJSON("https://raw.githubusercontent.com/kiang/pharmacies/master/json/points.json", function (response, status) {
...
function createList(data, target) {
    ...
    let dataFilter = [];
    for (let i = 0; i < data.length; i++) {
        if (data[i].properties.county == citySelect.value && data[i].properties.town == target.target.value) {
